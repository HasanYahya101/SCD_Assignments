...
public class SeeExpiredCNIC extends JPanel {
    ...
    private void init() {
        ...
        int dataSize = nadra_data.size();
        Object[][] data = new Object[dataSize][5]; // dataSize rows, 5 columns
        int index = 0;

        int count = 0;

        if (nadra_data.size() > 0) {
            for (NADRA_Data nadraData : nadra_data) {
                ...
                if (flag) {
                    ...
                    Customer_Data temp_customer = (Customer_Data) cl
                            .sendRequest("customer.getCustomer," + nadraData.CNIC);
        ...
        if (count == 0) {
            JOptionPane.showMessageDialog(null, "No expired CNIC's found", "Info",
